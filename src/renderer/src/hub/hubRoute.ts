...
//
//   #/browse
//   #/c/<conceptId>[/v/<canvas|wiki|table>][/cv/<namedViewId>][/cat/<category>][/tag/<tag,tag>][/status/<status,status>][/sort/<name|category|connections>]
//
...
export interface HubRoute {
  ...
  concept?: string
  view?: HubViewKind
  /** One of the concept's named views, in the mode given by `view`. */
  namedView?: string
  category?: string
...
export function parseHubHash(hash: string): HubRoute {
  ...
  const namedView = map.cv || undefined
...
export function formatHubHash(route: HubRoute): string {
  ...
  if (route.concept && route.view && route.namedView) segs.push('cv', encodeURIComponent(route.namedView))
